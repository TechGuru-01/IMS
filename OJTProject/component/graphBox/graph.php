...
// Default to Current Month/Year if session is empty (Sync with inventory_logic.php)
$m = $_SESSION['selected_month'] ?? (int)date('n');
$y = $_SESSION['selected_year'] ?? (int)date('Y');
$view = $_GET['view'] ?? 'weekly';

// --- 2. SQL LOGIC (Improved for Performance) ---
// Siguradong may value na ang $m dito dahil sa fallback sa itaas
...
if ($chartResult) {
    while ($row = $chartResult->fetch_assoc()) {
        ...
        if ($view === 'weekly') {
            // Sinisiguro na 0-6 index para sa Mon-Sun mapping
            $allChartData[$itemName]['stats'][$row['day_index']] = (int)$row['usage_count'];
        } else {
            // Para sa monthly view, gagamitin ang date as key
...
<script>
    // Ipasa ang data sa Chart.js o kahit anong library na gamit mo
...
<div class="box-content box-4" id="box-graph">
    <div class="content-container" id="graph-content" style="height: 100%;">
        ...
        <div id="chartsWrapper" style="padding: 15px;">
            ...
            <?php else: ?>
                <p style="font-size: 0.8rem; color: #666;">Showing top item usage trends.</p>
            <?php endif; ?>
        </div>
    </div>
</div>