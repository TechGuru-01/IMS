<?php
$cols = ['pr_id', 'ref_number', 'pr_date', 'status', 'company', 'all_materials', 'quantity_req', 'unit_price', 'total_amount', 'remarks'];
?>
...
<table class="inventory-table" id="inventoryTable">
    ...
    <?php 
        // Kunin ang class base sa status para sa row coloring
        $row_status_class = getStatusClass($row['status'] ?? ''); 
    ?>
    <tr data-id="<?= $row['pr_id'] ?>" class="<?= $row_status_class ?>">
                    <?php foreach ($cols as $col): ?>
                        <?php if (strtolower($col) == 'id' || strtolower($col) == 'pr_id') continue; ?> 
                        
                        <td data-field="<?= $col ?>">
                            <?php 
                                // --- 1. MANUAL STATUS DROPDOWN LOGIC ---
                                if ($col == 'status') {
                                    ...
                                    $status_options = [
                                                    ...
                                                    'Production Office', 
                                                    'Received', 
                                                    'Ready for Reporting', // Eto yung bago
                                                    'Cancelled'
                                                     ];
                                    $status_class = getStatusClass($current_status); // Function from main page
