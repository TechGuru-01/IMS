document.addEventListener("DOMContentLoaded", () => {
  ...
  const triggerModal = () => {
    ...
    // Kunin ang headers mula sa data-column attribute
    ...
    tableClone.querySelectorAll("tbody tr").forEach((row) => {
      ...
      row.querySelectorAll("td").forEach((td, i) => {
        ...
        // Protektado dapat ang mga system columns at UUID
        const readOnlyCols = [
          "select",
          "id",
          "total_value",
          ...
          "received_qty",
          "is_acknowledged",
          "item_uuid", // Para hindi mabura ang link ng QR
        ];
    ...
    // Hinahanap ang "Generate QR" button sa loob ng modal
    const qrBtnContainer = document.querySelector(".qr-btn-container");
    if (qrBtnContainer) {
      const qrLink = qrBtnContainer.closest("a"); // Kunin ang parent link tag

      if (qrLink) {
        qrLink.onclick = (e) => {
          // Pigilan muna ang automatic redirect
          e.preventDefault();

          // Kunin ang mga checkboxes sa cloned table (editorTable)
          ...
          if (selectedIds.length === 0) {
            ...
            // Kunin ang base href at dagdagan ng IDs
