...
if (isset($_POST['addItem'])){
    ...
    $finalStock = $oldQty + $input_qty; 


    // --- UUID LOOKUP: NUUID ---
    $stmtUUID = $conn->prepare("SELECT item_uuid FROM inventory WHERE item = ? AND description = ? AND item_uuid IS NOT NULL AND item_uuid != '' LIMIT 1");
    $stmtUUID->bind_param("ss", $item, $description);// changed the cabinet to description
    $stmtUUID->execute();
