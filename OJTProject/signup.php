...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // 1. Linisin ang input
    $fullname = trim($_POST['full_name']); // Bagong field
    $user = trim($_POST['username']);
    $pass = $_POST['password'];
    $confirm_pass = $_POST['confirm_password'];

    // 2. Check kung empty (Sinama na natin ang $fullname)
    if (!empty($fullname) && !empty($user) && !empty($pass) && !empty($confirm_pass)) {
        ...
        if ($pass !== $confirm_pass) {
            ...
            // 5. Isinama ang full_name sa prepared statement
            // Siguraduhin na may column na 'full_name' sa database mo!
...
<head>
    ...
    <style>
        /* Konting dagdag na style para sa messages */
    ...
    <div class="login-wrapper">
        ...
        <div class="form-side">
            ...
            <p style="text-align:center; margin-top:20px; font-size:0.9rem; color:#666;">
                Already have an account? <a href="login.php" style="color:#d32f2f; text-decoration:none; font-weight:bold;">Login</a>
            </p>
        </div>
    </div>
